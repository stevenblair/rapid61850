...
### EMF import process ###
 ...
 5. Create a new project of type "Convert Projects to JET Projects" (in newer versions of Eclipse, this is from the `New > Other...` menu), and select the `emf` project. For the `emf` project, go to Project Properties > JET Settings, and set Template Containers to "templates", and Source Container to "src". You can delete the `rapid61850/emf/rapid61850/templates` directory (but not the `rapid61850/emf/templates` or `rapid61850/emf/src/rapid61850/templates` directories) that was created before JET was configured correctly.
...
An example SCD file and a `main.c` file are provided. Many of the other C files are generated automatically. For the C code to compile with Eclipse, you should:
 ...
 - Install MinGW and add `C:\MinGW\bin;` to `PATH` in the Project Properties > C/C++ Build > Environment options. Other compilers should work too. It's possible to use the 64-bit TDM-GCC compiler for Windows, but you should use npcap instead of Winpcap, and first convert the library files in `c\Lib` as given here: https://github.com/magisterquis/EDRSniper/blob/master/build.bat
