...
### EMF import process ###
 ...
 3. Select the three root packages that are imported (although, only `scl` is used). Click "Finish". This will regenerate some files in `emf/model`: scl.ecore, lcoordinates.ecore, lmaintenance.ecore, and SCL.genmodel.
 ...
 6. The package `org.eclipse.em.query` may need to be add to the Plug-in Dependencies. This can be done from the mouse cursor tooltip for the compiler error at the `import` statements.
